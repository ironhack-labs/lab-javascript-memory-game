    ...
    card.addEventListener('click', () => {
      ...
      if (memoryGame.pickedCards.length === 2) {
        ...
        if (isPair) {
          ...
          const userWon = memoryGame.checkIfFinished();
          if (userWon) setTimeout(() => {document.getElementById('memory-board').innerHTML = `
            <div>
              <h1>Congratulations, you won!</h1>
              <h2>Score</h2>
              <p>Pairs clicked: ${memoryGame.pairsClicked}</p>
              <p>Pairs guessed: ${memoryGame.pairsGuessed}</p>
            </div>
          `}, 1000);
        } else {
          setTimeout(() => {
