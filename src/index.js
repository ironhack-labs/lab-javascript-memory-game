  ...
  document.querySelectorAll(".card").forEach((card) => {
    card.addEventListener("click", () => {
      ...
      if (memoryGame.pickedCards.length === 2) {
        const [firstPickedCard, secondPickedCard] = memoryGame.pickedCards;
        const firstCardName = firstPickedCard.getAttribute("data-card-name");
        const secondCardName = secondPickedCard.getAttribute("data-card-name");

        const isMatch = memoryGame.checkIfPair(firstCardName, secondCardName);

        if (!isMatch) {
          setTimeout(() => {
            firstPickedCard.classList.toggle("turned");
            secondPickedCard.classList.toggle("turned");
          }, 800);
        }
